...
class TextController extends SimpleController
{
    ...
    public function pageList($request, $response, $args)
    {
        ...
        return $this
            ->ci
            ->view
            ->render($response, 'pages/texts.html.twig');
    }
    ...
    public function create($request, $response, $args)
    {
        ...
        Capsule::transaction(function () use ($classMapper, $data, $ms, $config, $currentUser)
        {
            ...
            $this
                ->ci
                ->userActivityLogger
                ->info("User {$currentUser->user_name} created a new text with the technical name {$text->technical_name}.", ['type' => 'text_created', 'user_id' => $currentUser->id]);
    ...
    protected function getTextFromParams($params)
    {
        ...
        $text = $classMapper->staticMethod('text', 'where', 'id', $data['text_id'])->with('creator')
            ->first();
    ...
    public function delete($request, $response, $args)
    {
        ...
        Capsule::transaction(function () use ($text, $title, $currentUser)
        {
            ...
            $this
                ->ci
                ->userActivityLogger
                ->info("User {$currentUser->user_name} deleted the text with the technical name {$title}.", ['type' => 'text_deleted', 'user_id' => $currentUser->id]);
        });

        $ms->addMessageTranslated('success', 'TEXT.DELETION_SUCCESSFUL', ['name' => $title]);
    ...
    public function editForm($request, $response, $args)
    {
        ...
        $text = $classMapper->staticMethod('text', 'where', 'id', $text->id)
            ->first();
